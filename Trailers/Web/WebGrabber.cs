...
namespace Trailers.Web
{
    public class WebGrabber
    {
        ...
        public string UserAgent
        {
            ...
            set { userAgent = value; }
        } private string userAgent = "Trailers/" + Assembly.GetExecutingAssembly().GetName().Version.ToString();
        ...
        public bool GetResponse()
        {
            try
            {
                ...
                while (!completed)
                {
                    ...
                    catch (WebException e)
                    {
                        ...
                        if (e.Status == WebExceptionStatus.ProtocolError)
                        {
                            ...
                            switch (statusCode)
                            {
                                ...
                                default:
                                    FileLog.Error("Connection failed: URL='{0}', Status='{1}', Description='{2}'.", requestUrl, statusCode, ((HttpWebResponse)e.Response).StatusDescription);
                                    return false;
                            }
                        }
                        ...
                        {
                            FileLog.Warning("Connection failed: Reached retry limit of '" + maxRetries + "'. URL=" + requestUrl);
                            return false;
                        }
                    ...
                    {
                        FileLog.Error("Connection failed.", e.Message);
                        return false;
                    }
                ...
                // Debug
                if (_debug)
                    FileLog.Debug("GetResponse: URL={0}, UserAgent={1}, CookieHeader={2}, Accept={3}", requestUrl, userAgent, cookieHeader, _accept);
            ...
            {
                FileLog.Warning("Unexpected error getting http response from '{0}': {1}", requestUrl, e.Message);
                return false;
            }
        }
        ...
        public string GetString()
        {
            ...
            if (encoding == null)
            {
                ...
                catch (Exception e)
                {
                    ...
                    FileLog.Debug("Encoding could not be determined, using default.", e.Message);
            ...
            // Debug
            if (_debug) 
                FileLog.Debug("GetString: Encoding={2}", encoding.EncodingName);
            ...
            catch (Exception e)
            {
                ...
                FileLog.Error("Error while trying to read stream data: ", e.Message);
            }
        ...
        public XmlNodeList GetXML(string rootNode)
        {
            ...
            {
                FileLog.Error("XML Parse error: URL=" + requestUrl, e.Message);
                return null;
            }
        ...
        private bool SetAllowUnsafeHeaderParsing(bool setState)
        {
            ...
            catch (Exception e)
            {
                ...
                FileLog.Error("Unsafe header parsing setting change failed.");
                return false;
            }
        }
