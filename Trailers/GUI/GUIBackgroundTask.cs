...
namespace Trailers.GUI
{
    internal class GUIBackgroundTask
    {
        ...
        System.Timers.Timer timeoutTimer = new System.Timers.Timer(PluginSettings.WebTimeout * PluginSettings.WebMaxRetries + (PluginSettings.WebTimeoutIncrement * PluginSettings.WebMaxRetries)) { AutoReset = false };
        ...
        void StopBackgroundTask(bool byUserRequest)
        {
            ...
            {
                FileLog.Info("Aborting background thread: {0}", _CurrentTaskDescription);
        ...
        internal bool ExecuteInBackgroundAndCallback(Func<object> task, Action<bool, object> resultHandler, string taskDescription, bool timeout)
        {
            ...
            if (!IsBusy && Monitor.TryEnter(this))
            {
                try
                {
                    ...
                    backgroundThread = new Thread(delegate()
                    {
                        ...
                        {
                            if (!abortedByUser) 
                                FileLog.Info("Timeout waiting for results.");
                        ...
                        catch (Exception threadException)
                        {
                            ...
                            FileLog.Info(threadException.ToString());
            ...
            {
                FileLog.Info("Another thread tried to execute a task in background.");
                return false;
            }
        }
